...
public class ImageTool {
	 ...
	 * 
	 * @param srcPath
	 *            return Map { height=2336, width=4160, originalTime=2015:01:21 12:15:33,
	...
	public static Map caseHandle(String srcPath) {
		...
		try {
			...
			returnMap = Img4Util.getImageInfo(srcPath);
	...
	public static Map portraitHandle(String srcPath) {
		...
		try {
			...
			String dest_compress_path = StringTool.replace(srcPath, FastDFSUtil.DIAN_SEPARATOR,
							FastDFSUtil.UNDERLINE_SEPARATOR
									+ FastDFSUtil.COMPRESS
									+ FastDFSUtil.DIAN_SEPARATOR);

			 Img4Util.compressImage(
	...
	private static String zoomAndUpload(String[] uploadResults, String srcPath,
		...
		Img4Util.zoomImage(destWidth, destHeight, srcPath, destPath,extName);
	...
	private static String cropAndUpload(String[] uploadResults, String srcPath,
		...
		Img4Util.cropImage(srcPath, destPath, x_start, y_start, x_end,
				y_end, extName);
	...
	private static String[] handle_special(String[] uploadResults,
		...
		try {
			...
			if (isZoom && isCrop) {
				...
				Img4Util.zoomImage(zoomWidth, zoomHeight, srcPath,
						destPath_temp,extName);
				...
				Img4Util.cropImage(destPath_temp, destPath, x_start,
				...
				Img4Util.cropImage(srcPath, destPath, x_start, y_start,
			...
			Map imageMap = Img4Util.getImageInfo(destPath);
	...
	private static Map getParamDynamic(int originalWidth, int originalHeight,
		...
		case 1:// 4000*1000
			...
			} else if (ImageTool.foramtNumber(originalWidth,
					originalHeight, 2) > 1
					&& ImageTool.foramtNumber(originalWidth,
							originalHeight, 2) < 2) {
				zoomWidth = limitHigh;
				zoomHeight = 0;
	...
	private static Map getParamTerminal(int originalWidth, int originalHeight,
		...
		case 1:// 4000*1000
			...
			System.out.println(">><<<<>>>>>>limitX/limitY="
					+ ImageTool.foramtNumber(limitX, limitY, 2));

			if (ImageTool.foramtNumber(originalWidth, originalHeight, 2) >= ImageTool
					.foramtNumber(limitX, limitY, 2)) {
				zoomWidth = 0;
				zoomHeight = limitY;
			} else if (ImageTool.foramtNumber(originalWidth,
					originalHeight, 2) < ImageTool.foramtNumber(limitX,
					limitY, 2)) {
				zoomWidth = limitX;
				zoomHeight = 0;
			}
