 ...
 * @date 2015-8-1
 */
@Component("orderProductResource")
@Path("/order/product")
public class OrderProductResource{
	...
	private static final Logger logger = Logger
			.getLogger(OrderProductResource.class);
	...
	public RepObject getById(@PathParam("id") String id) {
		...
		if(StringUtils.isNotEmpty(id)){
			HashMap item= orderProductService.getById(id);
			resp.setData(item);
	...
	public RepObject getPageList(@QueryParam("queryJson") String queryJson) {
		...
		Page page=orderProductService.getPageList(paramMap);
		resp.setData(page);
		return resp;
	}
	...
	public RepObject getList(@QueryParam("queryJson") String queryJson) {
		RepObject resp=new RepObject(true,"success","");
		Map paramMap=ConvertUtil.objToHashMap(queryJson);
		List<HashMap> list = orderProductService.getList(paramMap);
	...
	public RepObject getCount(@QueryParam("queryJson") String queryJson) {
		RepObject resp=new RepObject(true,"success","");
		Map paramMap=ConvertUtil.objToHashMap(queryJson);
		int total=orderProductService.getCount(paramMap);
		resp.setData(total);
		return resp;
	}
